...
tests dataV =
  HUnit.TestList
    [ HUnit.TestLabel "Client Creation" $ testClientCreation dataV,
      HUnit.TestLabel "PostMenu Operation" $ testPostMenu dataV
    ]

app :: State -> Wai.Application
app state request responder = do
  response <- Stm.atomically $ Stm.takeTMVar (res state)
  _ <- Stm.atomically $ Stm.writeTMVar (req state) request
  responder response
...
main = do
  ...
  _ <- forkIO (Warp.run port $ app state)
