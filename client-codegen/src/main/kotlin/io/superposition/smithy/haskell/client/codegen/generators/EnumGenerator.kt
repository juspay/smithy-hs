...
class EnumGenerator<T : ShapeDirective<Shape, HaskellContext, HaskellSettings>> : Consumer<T> {
    ...
    override fun accept(directive: T) {
        ...
        directive.context().writerDelegator().useShapeWriter(shape) { writer ->
            ...
            writer.putContext("constructors", Runnable { generateConstructors(writer, shape) })
            writer.putContext("derives", Runnable { generateDerives(writer) })
            writer.putContext("serializer", Runnable { serializerGenerator(writer, shape, directive.symbol()) })
            writer.putContext("deserializer", Runnable { generateDeserializers(writer, shape, symbol) })
    ...
    private fun generateDeserializers(
        ...
        writer.openBlock("instance #{aeson:N}.FromJSON ${symbol.name} where", "") {
            writer.openBlock("parseJSON = #{aeson:N}.withText ${symbol.name.dq()} $ \\v ->", "") {
