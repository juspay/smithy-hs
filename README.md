...
The `build` (`gradle build`) task runs `smithy-build` as part of the build process. So once the codegen plugin is compiled, it will automatically run the plugin & generate
code from a smithy model meant for testing. You can take a look at the `client-codegen-test` module.

## Testing

Run `cabal test` in `hs-it`, this will test the generated-client for the modeled service.
